...
export const projects: Project[] = [
  {
    ...
    description: 'Painel de operação desenvolvido para a Kuehne+Nagel, inspirado nos dashboards de centros de distribuição de grandes operadores logísticos.',
    longDescription: 'Painel de operação (Daily Operation Overview) desenvolvido para a Kuehne+Nagel, com visual inspirado nos dashboards de centros de distribuição de grandes operadores logísticos (Amazon, DHL, Mercado Livre). HTML5, CSS3 e JavaScript puro (vanilla) — nenhum framework. Google Fonts (Space Grotesk, Inter, JetBrains Mono). Ícones SVG inline, desenhados sob medida. API de armazenamento key-value para persistência dos dados (sem necessidade de banco de dados). Feito para ser exibido em TVs de sala de operação, atualizado em tempo real e editado por qualquer pessoa da equipe através de um painel administrativo, sem necessidade de mexer em código.',
    image: '/dailyopkn.png',
    tags: ['Vanilla JS', 'HTML5/CSS3', 'TV Dashboard', 'Logística'],
    metrics: [
      { label: 'Stack', value: 'Vanilla JS + HTML/CSS' },
      { label: 'Persistência', value: 'Key-Value API' },
      { label: 'Ambiente', value: 'TVs Operacionais' }
    ],
    client: 'Kuehne+Nagel',
    buyUrl: 'https://dailydashop.netlify.app/'
  },
  {
    id: '01',
